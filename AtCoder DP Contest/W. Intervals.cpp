...
struct node{long long v, lz;} seg[MM<<2];
int n, m; long long sum, dp;
...
void update(int i, int l, int r, long long v, int l1 = 0, int r1 = n + 1){
...
long long get(int i, int l, int r, int l1 = 0, int r1 = n + 1){
...
int main(){
	n = read(), m = read();
	for (int i = 0; i < m; i++) a[i] =  {read(), read(), read()}, sum += a[i].v;
